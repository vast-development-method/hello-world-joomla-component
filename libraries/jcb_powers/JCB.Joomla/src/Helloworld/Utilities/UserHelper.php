...
namespace JCB\Joomla\Helloworld\Utilities;
...
use JCB\Joomla\Helloworld\Utilities\Exception\NoUserIdFoundException;
...
/***[JCBGUI.power.main_class_code.821.$$$$]***/
		...
		array $params = ['useractivation' => 0, 'sendpassword' => 1], int $mode = 1): int
	{
		...
		{
			throw new \InvalidArgumentException(Text::_('COM_HELLOWORLD_CAN_NOT_SAVE_USER_WITHOUT_EMAIL_VALUE'));
		}
		...
		array $params = ['useractivation' => 0, 'sendpassword' => 1], int $mode = 1): int
	{
		...
		if ($mode === 2 && method_exists(HelloworldHelper::class, 'registerUser'))
		{
			$params['autologin'] = $autologin;
			$userId = HelloworldHelper::registerUser($credentials, $params);
			...
			throw new NoUserIdFoundException(Text::_('COM_HELLOWORLD_USER_CREATION_FAILED'));
		}
		...
		throw new NoUserIdFoundException(
			Text::sprintf('COM_HELLOWORLD_USER_S_S_CREATION_FAILEDS',
	...
	public static function update(array $userDetails): int
	{
		...
		throw new \RuntimeException(
			Text::sprintf('COM_HELLOWORLD_UPDATE_OF_USER_S_S_FAILEDS',
