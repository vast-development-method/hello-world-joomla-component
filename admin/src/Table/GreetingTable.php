...
/------------------------------------------------------------------------------------------------------*/
namespace JCB\Component\Helloworld\Administrator\Table;
 ...
 * @since  1.6
 */
class GreetingTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_helloworld.greeting';
		...
		parent::__construct('#__helloworld_greeting', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Greeting table.
	 *
	...
	public function store($updateNulls = true)
	{
		...
		{
			// New greeting. A greeting created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_HELLOWORLD_GREETING_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_HELLOWORLD_GREETING_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_helloworld.greeting.'.$this->id);
			$this->setRules($rules);
		}
	...
	protected function _getAssetParentId(?Table $table = null, $id = null)
	{
		...
		// load the helloworld asset
		$assets->loadByName('com_helloworld');
