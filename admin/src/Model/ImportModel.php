...
/***
 * Helloworld Import Base Database Model
 *
 * @since  1.6
 */
class ImportModel extends BaseDatabaseModel
{
	 ...
	 * @var        string
	 */
	protected $_context = 'com_helloworld.import';
	...
	protected function populateState()
	{
		...
		$this->setState('message', $app->getUserState('com_helloworld.message'));
	...
	public function import()
	{
		...
		if ($package === null)
		{
			switch ($this->getType)
			{
				...
				default:
					$app->setUserState('com_helloworld.message', Text::_('COM_HELLOWORLD_IMPORT_NO_IMPORT_TYPE_FOUND'));
		...
		if (!$package || !$package['type'])
		{
			...
			$app->setUserState('com_helloworld.message', Text::_('COM_HELLOWORLD_IMPORT_UNABLE_TO_FIND_IMPORT_PACKAGE'));
			return false;
		}
		...
		if (!$this->setData($package,$this->dataType,$headerList))
		{
			...
			$msg = Text::_('COM_HELLOWORLD_IMPORT_ERROR');
			$back = $session->get('backto_VDM_IMPORT', NULL);
			if ($back)
			{
				$app->setUserState('com_helloworld.redirect_url', 'index.php?option=com_helloworld&view='.$back);
		...
		else
		{
			...
			$msg = Text::sprintf('COM_HELLOWORLD_IMPORT_SUCCESS', $package['packagename']);
			$back = $session->get('backto_VDM_IMPORT', NULL);
			if ($back)
			{
				$app->setUserState('com_helloworld.redirect_url', 'index.php?option=com_helloworld&view='.$back);
	...
	protected function _getPackageFromUpload()
	{
		...
		{
			$app->enqueueMessage(Text::_('COM_HELLOWORLD_IMPORT_MSG_WARNIMPORTFILE'), 'warning');
			return false;
		}
		...
		{
			$app->enqueueMessage(Text::_('COM_HELLOWORLD_IMPORT_MSG_NO_FILE_SELECTED'), 'warning');
			return false;
		}
		...
		{
			$app->enqueueMessage(Text::_('COM_HELLOWORLD_IMPORT_MSG_WARNIMPORTUPLOADERROR'), 'warning');
			return false;
		}
	...
	protected function _getPackageFromFolder()
	{
		...
		{
			$app->enqueueMessage(Text::_('COM_HELLOWORLD_IMPORT_MSG_PLEASE_ENTER_A_PACKAGE_DIRECTORY'), 'warning');
			return false;
		}
		...
		if (!$type)
		{
			$app->enqueueMessage(Text::_('COM_HELLOWORLD_IMPORT_MSG_PATH_DOES_NOT_HAVE_A_VALID_PACKAGE'), 'warning');
		}
		...
		{
			// set error message
			$app->enqueueMessage(Text::_('COM_HELLOWORLD_IMPORT_MSG_DOES_NOT_HAVE_A_VALID_FILE_TYPE'), 'warning');
			return false;
		}
	...
	protected function _getPackageFromUrl()
	{
		...
		if (!$url)
		{
			$app->enqueueMessage(Text::_('COM_HELLOWORLD_IMPORT_MSG_ENTER_A_URL'), 'warning');
			return false;
		}
		...
		if (!$p_file)
		{
			$app->enqueueMessage(Text::_('COM_HELLOWORLD_IMPORT_MSG_INVALID_URL'), 'warning');
			return false;
		}
	...
	**/
	protected function setData($package,$table,$target_headers)
	{
		if (UtilitiesArrayHelper::check($target_headers))
		{
			...
			HelloworldHelper::composerAutoload('phpspreadsheet');
	...
	**/
	protected function save($data,$table)
	{
		...
		if(UtilitiesArrayHelper::check($data['array']))
		{
			...
			if(UtilitiesArrayHelper::check($data['array']) && $user->authorise($table.'.import', 'com_helloworld') && $user->authorise('core.import', 'com_helloworld'))
			{
				// set target.
				...
				$canDo			= HelloworldHelper::getActions($table);
				...
				foreach($data['array'] as $row)
				{
					...
					if (isset($row[$id_key]) && is_numeric($row[$id_key]) && $row[$id_key] > 0)
					{
						...
						$query
							->select('version')
							->from($db->quoteName('#__helloworld_'.$table))
					...
					if($found && $canEdit)
					{
						...
						$query->update($db->quoteName('#__helloworld_'.$table))->set($fields)->where($conditions);
						$db->setQuery($query);
						$db->execute();
					}
					elseif ($canCreate)
					{
						...
						$query
							->insert($db->quoteName('#__helloworld_'.$table))
						...
						if ($done)
						{
							...
							HelloworldHelper::setAsset($aId,$table);
						}
					}
					else
					{
	...
	protected function getAliasesUsed($table)
	{
		...
		$columns = $db->getTableColumns('#__helloworld_'.$table);
		if(isset($columns['alias'])){
			// Create a new query object.
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('alias')));
			$query->from($db->quoteName('#__helloworld_'.$table));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
